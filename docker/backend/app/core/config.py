...
class IncidentConfig:
    consecutive_ticks_to_open: int
    consecutive_ticks_to_recover: int
...
class Settings(BaseSettings):
    ...
    # Gates whether MonitorScheduler starts any monitor at all (Step 6+).
    # Defaults on for real usage. Tests that only exercise the HTTP/WS
    # layers explicitly set this false, so building a FastAPI app in a
    # test doesn't incidentally start real background network pinging.
    ...
    ping_timeout_seconds: float = Field(default=1.0, gt=0)
    # Number of recent probes (per target) kept in memory to compute
    # jitter/rolling average/packet-loss %. 30 samples x 2s interval =
    # a 1-minute rolling window — long enough to smooth single-probe
    # noise, short enough that a real recovery shows up within a minute
    # rather than being dragged down by stale data.
    ping_rolling_window_size: int = Field(default=30, gt=1)
    # icmplib "privileged" mode needs a raw ICMP socket, which needs
    # root (Linux/macOS) or Administrator (Windows). Set to false only
    # if the process truly can't get elevated privileges — unprivileged
    # mode is slower and, per icmplib's own docs, not supported on
    # Windows at all.
    ...
    # --- Incident Engine -------------------------------------------------
    # Debounce: how many consecutive bad/good ticks are needed before an
    # incident opens/recovers. At the default 2s ping interval, 3 ticks
    # is ~6 seconds — long enough that a single dropped packet doesn't
    # open an incident, short enough that a real outage is caught fast.
    incident_consecutive_ticks_to_open: int = Field(default=3, gt=0)
    incident_consecutive_ticks_to_recover: int = Field(default=3, gt=0)
    ...
    def ping(self) -> PingConfig:
        return PingConfig(
            ...
            rolling_window_size=self.ping_rolling_window_size,
    ...
    def incident(self) -> IncidentConfig:
        return IncidentConfig(
            consecutive_ticks_to_open=self.incident_consecutive_ticks_to_open,
            consecutive_ticks_to_recover=self.incident_consecutive_ticks_to_recover,
        )
